...
echo "Make directory /var/data/project-plus-dolphinONPM/user/Wii if it doesn't exist"
mkdir -p /var/data/project-plus-dolphinONPM/Wii

echo "Make directory /var/config/project-plus-dolphinONPM if it doesn't exist"
mkdir -p /var/config/project-plus-dolphinONPM

# Create and set variables for the system and user SD card creation dates
# SystemSDCardCreationDate=`stat --format="%W" /app/share/project-plus-dolphinONPM/sys/Load/WiiSD.raw`
# UserSDCardCreationDate=`stat --format="%W" /var/data/project-plus-dolphinONPM/Load/WiiSD.raw`

# echo "Check if there is a newer SD card version"
# if [ $SystemSDCardCreationDate -gt ${UserSDCardCreationDate:=0} ];
# then
#     echo "Make directory /var/data/project-plus-dolphinONPM/Load if it doesn't exist"
...
UserHDTexturesCreationDate=`stat --format="%W" /var/data/project-plus-dolphinONPM/user/Load/Textures/RSBE01`

echo "Check if there are newer HD textures"
if [ $SystemHDTexturesCreationDate -gt ${UserHDTexturesCreationDate:=0} ];
then
    echo "Make directory /var/data/project-plus-dolphinONPM/user/Load/Textures if it doesn't exist"
