...
BINFILE=Project+_DolphinONPM-x86_64.AppImage
...
cp -P "$BUILDBIN"/project-plus-dolphinONPM $HOME/squashfs-root/usr/bin/

curl -sL https://raw.githubusercontent.com/WoodleyBrew/Project-Plus-DolphinONPM/$branch/Data/project-plus-dolphinONPM.svg -o ./squashfs-root/project-plus-dolphinONPM.svg
curl -sL https://raw.githubusercontent.com/WoodleyBrew/Project-Plus-DolphinONPM/$branch/Data/project-plus-dolphinONPM.desktop -o ./squashfs-root/project-plus-dolphinONPM.desktop
curl -sL https://github.com/AppImage/AppImageKit/releases/download/continuous/runtime-x86_64 -o ./squashfs-root/runtime
mkdir -p squashfs-root/usr/share/applications && cp ./squashfs-root/project-plus-dolphinONPM.desktop ./squashfs-root/usr/share/applications
mkdir -p squashfs-root/usr/share/icons && cp ./squashfs-root/project-plus-dolphinONPM.svg ./squashfs-root/usr/share/icons
mkdir -p squashfs-root/usr/share/icons/hicolor/scalable/apps && cp ./squashfs-root/project-plus-dolphinONPM.svg ./squashfs-root/usr/share/icons/hicolor/scalable/apps
mkdir -p squashfs-root/usr/share/pixmaps && cp ./squashfs-root/project-plus-dolphinONPM.svg ./squashfs-root/usr/share/pixmaps
mkdir -p squashfs-root/usr/optional/ ; mkdir -p squashfs-root/usr/optional/libstdc++/

mkdir -p squashfs-root/usr/share/project-plus-dolphinONPM
cp -R /dolphin/dolphin/Data/Sys ./squashfs-root/usr/bin
curl -sL "https://raw.githubusercontent.com/WoodleyBrew/Project-Plus-DolphinONPM/$branch/Distribution/appimage/update.sh" -o $HOME/squashfs-root/update.sh
curl -sL "https://raw.githubusercontent.com/WoodleyBrew/Project-Plus-DolphinONPM/$branch/Distribution/appimage/AppRun" -o $HOME/squashfs-root/AppRun
...
# /tmp/squashfs-root/AppRun $HOME/squashfs-root/usr/bin/project-plus-dolphinONPM -appimage -unsupported-allow-new-glibc -no-copy-copyright-files -no-translations -bundle-non-qt-libs
/tmp/squashfs-root/AppRun $HOME/squashfs-root/usr/bin/project-plus-dolphinONPM -unsupported-allow-new-glibc -no-copy-copyright-files -no-translations -bundle-non-qt-libs
...
# Package AppImage
/tmp/squashfs-root/usr/bin/appimagetool $HOME/squashfs-root -u "gh-releases-zsync|qurious-pixel|dolphin|continuous|Project+_DolphinONPM-x86_64.AppImage.zsync"
